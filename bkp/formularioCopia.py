...
def impressao(VAR_DESCRICAO_SERVICO, VAR_DESCRICAO_DO_SERVIÇO_outros_, VAR_DESCRICAO_DO_SERVIÇO_outros_complemento_, VAR_NOME_PROPRIETARIO, VAR_CPF, VAR_ENDERECO_PROPRIETARIO, VAR_N_PROPRIETARIO, VAR_TELEFONE, VAR_MUNICIPIO_PROPRIETARIO, VAR_CONTATO, VAR_EMAIL, VAR_N_CAR, VAR_NOME_IMOVEL, VAR_ENDERECO_DO_IMOVEL, VAR_N_DO_IMOVEL, VAR_MUNICIPIO_DO_IMOVEL, VAR_BAIRRO_PROPRIETARIO, VAR_BAIRRO_DO_IMOVEL):
    ...
    pdf.drawString(275,790, 'SECRETARIA DE ESTADO DE MEIO AMBIENTE – SEMA')
    pdf.drawString(285,780, 'INSTITUTO DE MEIO AMBIENTE DO ACRE – IMAC')
    pdf.drawString(272,770, 'ESCRITÓRIO TÉCNICO DE GESTÃO DO CAR E PRA-ACRE')
    ...
    if window == janela1 and event == 'Imprimir':
        ...
        if se_imprimir >= 20:
            ...
            if texto_cortado[2] == '':
                ...
                impressao(
                    VAR_DESCRICAO_SERVICO, 
                    VAR_DESCRICAO_DO_SERVIÇO_outros_[:58], 
                    VAR_DESCRICAO_DO_SERVIÇO_outros_complemento_[:67], 
                    VAR_NOME_PROPRIETARIO[:40], 
                    VAR_CPF[:14], 
                    VAR_ENDERECO_PROPRIETARIO[:60], 
                    VAR_N_PROPRIETARIO[:11], 
                    VAR_TELEFONE[:13], 
                    VAR_MUNICIPIO_PROPRIETARIO[:59], 
                    VAR_CONTATO[:13], 
                    VAR_EMAIL, 
                    VAR_N_CAR[:43], 
                    VAR_NOME_IMOVEL[:51], 
                    VAR_ENDERECO_DO_IMOVEL[:60], 
                    VAR_N_DO_IMOVEL[:11], 
                    VAR_MUNICIPIO_DO_IMOVEL[:59], 
                    VAR_BAIRRO_PROPRIETARIO[:46], 
                    VAR_BAIRRO_DO_IMOVEL[:46])
